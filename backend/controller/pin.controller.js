...
export const createPin = async (req, res) => {
  ...
  if (parsedCanvasOptions.size !== "original") {
    ...
    parsedCanvasOptions.originalOrientation === originalOrientation
      ? (clienAspectRatio = originalOrientation)
      : (clienAspectRatio = 1 / originalOrientation);
  }
  width = metadata.width;
  height = metadata.width / clienAspectRatio;
  ...
  const transformationString = `w-${width},h-${height}${
    originalAspectRatio > clienAspectRatio ? ",cm-pad_resize" : ""
...
export const interact = async (req,res) =>{
  ...
  if(type === "like") {
    ...
    const isSaved = await Like.findOne({
      pin: id,
      user: req.userId,
    });
    if (isSaved) {
